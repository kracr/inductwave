...
def ground_queries(dataset, query_structure, ent_in, ent_out, small_ent_in, small_ent_out, gen_num, max_ans_num,
    ...
    while num_sampled < gen_num:
        ...
        if len(answer_set) == 0:
            num_empty += 1
            continue
        if mode != 'train1' and mode != 'train2' and mode != 'train3':
...
def generate_queries(dataset, query_structures, gen_num, max_ans_num, gen_train, gen_valid, gen_test, query_names,
    ...
    # indexified_files = ['train_indexified.txt', 'valid_indexified.txt', 'test_indexified.txt']
    train_triples1, train_triples2, train_triples3, valid_triples, test_triples = dataset
    #
    if gen_train or gen_valid:
        ...
        train_ent_in1, train_ent_out1 = construct_graph_from_triples(train_triples1)  # ent_in
        train_ent_in2, train_ent_out2 = construct_graph_from_triples(train_triples2)  # ent_in
        train_ent_in3, train_ent_out3 = construct_graph_from_triples(train_triples3)  # ent_in
    if gen_valid or gen_test:
        ...
        valid_ent_in, valid_ent_out = construct_graph_from_triples(np.concatenate([train_triples1, train_triples2, train_triples3, valid_triples[0]], axis=0).tolist())
        ...
        full_val_in, full_val_out = construct_graph_from_triples(
            np.concatenate([
                train_triples1, train_triples2, train_triples3, valid_triples[0], valid_triples[1]
            ], axis=0).tolist()
        )
    if gen_test:
        ...
        test_ent_in, test_ent_out = construct_graph_from_triples(np.concatenate([train_triples1, train_triples2, train_triples3, test_triples[0]], axis=0).tolist())
        ...
        full_test_in, full_test_out = construct_graph_from_triples(
            np.concatenate([
                train_triples1, train_triples2, train_triples3, test_triples[0], test_triples[1]
            ], axis=0).tolist()
        )
    ...
    train_queries1 = defaultdict(set)
    train_tp_answers1 = defaultdict(set)
    train_fp_answers1 = defaultdict(set)
    train_fn_answers1 = defaultdict(set)
    train_queries2 = defaultdict(set)
    train_tp_answers2 = defaultdict(set)
    train_fp_answers2 = defaultdict(set)
    train_fn_answers2 = defaultdict(set)
    train_queries3 = defaultdict(set)
    train_tp_answers3 = defaultdict(set)
    train_fp_answers3 = defaultdict(set)
    train_fn_answers3 = defaultdict(set)
    ...
    if query_structure == ['e', ['r']]:
        if gen_train:
            q, tp, fn, fp = write_links(dataset, train_ent_out1, defaultdict(lambda: defaultdict(set)), max_ans_num,
                        'train1-' + query_name)
            train_queries1.update(q)
            train_tp_answers1.update(tp)
            train_fn_answers1.update(fn)
            q, tp, fn, fp = write_links(dataset, train_ent_out2, defaultdict(lambda: defaultdict(set)), max_ans_num,
                        'train2-' + query_name)
            train_queries2.update(q)
            train_tp_answers2.update(tp)
            train_fn_answers2.update(fn)
            q, tp, fn, fp = write_links(dataset, train_ent_out3, defaultdict(lambda: defaultdict(set)), max_ans_num,
                        'train3-' + query_name)
            train_queries3.update(q)
            train_tp_answers3.update(tp)
            train_fn_answers3.update(fn)
        if gen_valid:
        ...
        return ([train_queries1, {list2tuple(query_structure): train_tp_answers1}, {list2tuple(query_structure): train_fn_answers1}],
                [train_queries2, {list2tuple(query_structure): train_tp_answers2}, {list2tuple(query_structure): train_fn_answers2}],
                [train_queries3, {list2tuple(query_structure): train_tp_answers3}, {list2tuple(query_structure): train_fn_answers3}],
    ...
    s0 = time.time()
    if gen_train:
        train_queries1, train_tp_answers1, train_fp_answers1, train_fn_answers1 = ground_queries(dataset, query_structure,
                                                                                             train_ent_in1,
                                                                                             train_ent_out1, defaultdict(
                lambda: defaultdict(set)), defaultdict(lambda: defaultdict(set)),
                                                                                             gen_num[0], max_ans_num,
                                                                                             query_name, 'train1',
                                                                                             ent2id, rel2id)
        train_queries2, train_tp_answers2, train_fp_answers2, train_fn_answers2 = ground_queries(dataset, query_structure,
                                                                                             train_ent_in2,
                                                                                             train_ent_out2, defaultdict(
                lambda: defaultdict(set)), defaultdict(lambda: defaultdict(set)),
                                                                                             gen_num[0], max_ans_num,
                                                                                             query_name, 'train2',
                                                                                             ent2id, rel2id)
        train_queries3, train_tp_answers3, train_fp_answers3, train_fn_answers3 = ground_queries(dataset, query_structure,
                                                                                             train_ent_in3,
                                                                                             train_ent_out3, defaultdict(
                lambda: defaultdict(set)), defaultdict(lambda: defaultdict(set)),
                                                                                             gen_num[0], max_ans_num,
                                                                                             query_name, 'train3',
                                                                                             ent2id, rel2id)
