...
class LogicalQuery(tasks.Task, core.Configurable):
    ...
    def forward(self, batch, eph=0):
        all_loss = torch.tensor(0, dtype=torch.float32, device=self.device)
        metric = {}

        pred, target = self.predict_and_target(batch, all_loss, metric, eph)
...
class InductiveLogicalQuery(LogicalQuery):
    ...
    def preprocess(self, train1_set, train2_set, train3_set, valid_set, test_set):
        if isinstance(train1_set, torch_data.Subset):
            dataset = train1_set.dataset
        else:
            dataset = train1_set
        self.num_entity = dataset.num_entity
        self.num_relation = dataset.num_relation
        self.id2type = dataset.id2type
        self.type2id = dataset.type2id
        self.register_buffer("train1_graph", dataset.train1_graph)
        self.register_buffer("train2_graph", dataset.train2_graph)
        self.register_buffer("train3_graph", dataset.train3_graph)
        ...
        return train1_set, train2_set, train3_set, valid_set, test_set

    def predict_and_target(self, batch, all_loss=None, metric=None, eph=0):
        query = batch["query"]
        type = batch["type"]
        easy_answer = batch["easy_answer"]
        hard_answer = batch["hard_answer"]
        if self.split=="train":
            if eph==0:
                graph = getattr(self, "%s1_graph" % self.split)
            elif eph==1:
                graph = getattr(self, "%s2_graph" % self.split)
            else:
                graph = getattr(self, "%s3_graph" % self.split)
        else:
            graph = getattr(self, "%s_graph" % self.split)
        pred = self.model(graph, query, self.split, all_loss, metric, eph=eph)
        if all_loss is None:
