...
def build_solver(cfg, dataset):
    ...
    if cfg.task.model["class"] == "InductWave":
        cfg.task.model.model.train1_graph = dataset.train1_graph
        cfg.task.model.model.train2_graph = dataset.train2_graph
        cfg.task.model.model.train3_graph = dataset.train3_graph
        cfg.task.model.model.valid_graph = dataset.valid_graph
        cfg.task.model.model.test_graph = dataset.test_graph
   
    train1_set, train2_set, train3_set, valid_set, test_set = dataset.split()
    if comm.get_rank() == 0:
        logger.warning(dataset)
        logger.warning("#train: %d, #valid: %d, #test: %d" % (len(train1_set), len(valid_set), len(test_set)))
    ...
    solver = core.Engine(task, train1_set, train2_set, train3_set, valid_set, test_set, optimizer, **cfg.engine)
