...
@R.register("dataset.InductiveWikiKGComp")
class InductiveWikiKGComp(LogicalQueryDataset):

    url = "https://zenodo.org/record/7306046/files/wikikg.zip"

    md5 = "fa30b189436ab46a2ff083dd6a5e6e0b"

    def __init__(self, path, ratio=0.6, inverse_projection=False, query_types=None, union_type="DNF", verbose=1,
...
class InductiveFB15k237Comp(LogicalQueryDataset):
    ...
    def __init__(self, path, ratio, inverse_projection=False, query_types=None, union_type="DNF", verbose=1,
        ...
        # Test inference ids: K+M .... N
        try:
            train_triplets1 = self.load_file(os.path.join(path, "train_graph1.txt"))
            train_triplets2 = self.load_file(os.path.join(path, "train_graph2.txt"))
            train_triplets3 = self.load_file(os.path.join(path, "train_graph3.txt"))
            val_inference = self.load_file(os.path.join(path, "val_inference.txt"))
            test_inference = self.load_file(os.path.join(path, "test_inference.txt"))
        except FileNotFoundError:
            print("Loading .pt files")
            train_triplets1 = self.load_pt(os.path.join(path, "train_graph1.pt"))
            train_triplets2 = self.load_pt(os.path.join(path, "train_graph2.pt"))
            train_triplets3 = self.load_pt(os.path.join(path, "train_graph3.pt"))
            val_inference = self.load_pt(os.path.join(path, "val_inference.pt"))
            test_inference = self.load_pt(os.path.join(path, "test_inference.pt"))

        entity_vocab, relation_vocab, tr_nodes1, tr_nodes2, tr_nodes3, vl_nodes, ts_nodes = self.build_vocab(train_triplets1, train_triplets2, train_triplets3, val_inference, test_inference)
        entity_vocab, inv_entity_vocab = self._standarize_vocab(entity_vocab, None)
        relation_vocab, inv_relation_vocab = self._standarize_vocab(relation_vocab, None)
        ...
        # Training graph: only training triples
        self.train1_graph = data.Graph(train_triplets1, num_node=np.max(tr_nodes1)+1, num_relation=num_relation)
        self.train2_graph = data.Graph(train_triplets2, num_node=np.max(tr_nodes2)+1, num_relation=num_relation)
        self.train3_graph = data.Graph(train_triplets3, num_node=np.max(tr_nodes3)+1, num_relation=num_relation)

        # Validation graph: training triples (0..K) + new validation inference triples (K+1...K+M)
        self.valid_graph = data.Graph(train_triplets1 + val_inference, num_node=num_node, num_relation=num_relation)

        # Test graph: training triples (0..K) + new test inference triples (K+M+1... N)
        self.test_graph = data.Graph(train_triplets1 + test_inference, num_node=num_node, num_relation=num_relation)

        # Full graph (aux purposes)
        self.graph = data.Graph(train_triplets1 + train_triplets2 + train_triplets3 + val_inference + test_inference,
                                num_node=num_node, num_relation=num_relation)
        self.entity_vocab = entity_vocab
        self.relation_vocab = relation_vocab
        self.inv_entity_vocab = inv_entity_vocab
        ...
        type2struct = {v: k for k, v in self.struct2type.items()}
        filtered_training_structs = tuple([type2struct[x] for x in train_patterns])
        for split in ["train1", "train2", "train3", "valid", "test"]:
            with open(os.path.join(path, "%s_queries.pkl" % split), "rb") as fin:
                struct2queries = pickle.load(fin)
            if split == "train1" or split == "train2" or split == "train3":
                query2hard_answers = defaultdict(lambda: defaultdict(set))
                with open(os.path.join(path, "%s_answers_hard.pkl" % split), "rb") as fin:
                    query2easy_answers = pickle.load(fin)
            else:
            ...
            for struct in structs:
                ...
                if query_type not in self.type2id:
                    continue
                # filter complex patterns ip, pi, 2u, up from training queries - those will be eval only
                if (split == "train1" or split == "train2" or split == "train3") and struct not in filtered_training_structs:
                    print(f"Skipping {query_type} - this will be used in evaluation")
                    continue
                struct_queries = sorted(struct2queries[struct])
                for query in struct_queries:
    ...
    def build_vocab(self, train_triples1, train_triples2, train_triples3, val_triples, test_triples):
    #def build_vocab(self, train_triples, val_triples, test_triples):
        all_triples = np.array(train_triples1+train_triples2+train_triples3+val_triples+test_triples)
        train_nodes1 = np.unique(np.array(train_triples1)[:, [0, 1]])
        train_nodes2 = np.unique(np.array(train_triples2)[:, [0, 1]])
        train_nodes3 = np.unique(np.array(train_triples3)[:, [0, 1]])
        val_nodes = np.unique(np.array(train_triples1 + train_triples2 + train_triples3 + val_triples)[:, [0, 1]])
        test_nodes = np.unique(np.array(train_triples1 + train_triples2 + train_triples3 + test_triples)[:, [0, 1]])
        num_entities = np.max(all_triples[:, [0, 1]]) + 1
        num_relations = np.max(all_triples[:, 2]) + 1
        ...
        return ent_vocab, rel_vocab, train_nodes1, train_nodes2, train_nodes3, val_nodes, test_nodes
