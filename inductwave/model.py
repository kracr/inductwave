...
class QueryExecutor(nn.Module, core.Configurable):
    ...
    def traversal_dropout(self, graph, h_prob, r_index):
        ...
        return graph.edge_mask(edge_mask), edge_mask

    def execute(self, graph, query, all_loss=None, metric=None, eph=0):
        ...
        while not op.is_stop().all():
            ...
            if not (is_operand | is_negation | is_intersection | is_union).any() and is_projection.any():
                ...
                self.apply_projection(is_projection, graph, r_index, all_loss=all_loss, metric=metric, eph=eph)
            op = query[all_sample, self.IP]
            #prof.step()
    ...
    def forward(self, graph, query, split, all_loss=None, metric=None, eph=0):
        self.split=split
        self.execute(graph, query, all_loss=all_loss, metric=metric, eph=eph)
    ...
    def apply_projection(self, mask, graph, r_index, all_loss=None, metric=None, eph=0):
        ...
        if all_loss is not None:
            ...
            graph, edge_mask = self.traversal_dropout(graph, sym_h_prob, r_index)

        # detach the variable to stabilize training
        h_prob = h_prob.detach()
        if self.split=='train':
            t_prob = self.model(graph, edge_mask, h_prob, r_index, self.split, all_loss=all_loss, metric=metric, eph=eph)
        else:
            t_prob = self.model(graph, None, h_prob, r_index, self.split, all_loss=all_loss, metric=metric)
...
class RelationProjection(nn.Module, core.Configurable):
    ...
    def __init__(self, model, num_mlp_layer=2):
        ...
        self.mlp = layers.MLP(model.output_dim, [64, 1])

    def forward(self, graph, edge_mask,  h_prob, r_index, split, all_loss=None, metric=None, eph=0):
        query = self.query(r_index)
        graph = graph.clone()
        with graph.graph():
            graph.query = query

        input = torch.einsum("bn, bd -> nbd", h_prob, query)#.to(torch.float16)
        output = self.model(graph, edge_mask, input, split,  all_loss=all_loss, metric=metric, eph=eph)
