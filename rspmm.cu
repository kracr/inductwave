...
void rspmm_forward_out_cuda(const int64_t *row_ptr, const int64_t *col_ind, const int64_t *layer_ind,
                            ...
                            scalar_t *output,
                            int64_t num_row, int64_t nnz, int64_t dim, int64_t value_dim) {
...
#pragma unroll
    ...
    for (int64_t block_ptr = ptr_start; block_ptr < ptr_end; block_ptr += warpSize) {
	    ...
	    int64_t current_ptr = block_ptr + offset_ptr;
#pragma unroll
            for (int64_t i = 0; i < kCoarseningFactor; i++) {
                int64_t d = d_start + i * warpSize;
                if (d >= dim)
                    break;
		scalar_t val = value[current_ptr * value_dim + (d % value_dim)]; 
...
#pragma unroll
		...
		int64_t v_idx = d % value_dim;
		scalar_t val = value[current_ptr * value_dim + v_idx];
		...
		atomicAdd(&value_grad[current_ptr * value_dim + v_idx], out_grad * dout_dy * dy_dval);
...
Tensor rspmm_forward_cuda(const SparseTensor &sparse, const Tensor &relation_, const Tensor &input_) {
    ...
    int64_t value_dim = value.size(1);
    ...
    AT_DISPATCH_FLOATING_TYPES(input.scalar_type(), "rspmm_forward_cuda", [&] {
        const int memory_size = kThreadPerBlock * (sizeof(int64_t) * 2);
