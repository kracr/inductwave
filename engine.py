...
class Engine(core.Configurable):
    ...
    def __init__(self, task, train1_set, train2_set, train3_set, valid_set, test_set, optimizer, scheduler=None, gpus=None, batch_size=1,
        ...
        if hasattr(task, "preprocess"):
            ...
            result = task.preprocess(train1_set, train2_set, train3_set, valid_set, test_set)
            if result is not None:
                train1_set, train2_set, train3_set, valid_set, test_set = result
        ...
        self.model = task
        self.train1_set = train1_set
        self.train2_set = train2_set
        self.train3_set = train3_set
    ...
    def train(self, num_epoch=1, batch_per_epoch=None):
        ...
        """
        sampler1 = torch_data.DistributedSampler(self.train1_set, self.world_size, self.rank)
        sampler2 = torch_data.DistributedSampler(self.train2_set, self.world_size, self.rank)
        sampler3 = torch_data.DistributedSampler(self.train3_set, self.world_size, self.rank)

        if self.rank==0:
            dataloader1 = data.DataLoader(self.train1_set, self.batch_size-1, sampler=sampler1, num_workers=self.num_worker)
            dataloader2 = data.DataLoader(self.train2_set, self.batch_size-1, sampler=sampler2, num_workers=self.num_worker)
            dataloader3 = data.DataLoader(self.train3_set, self.batch_size-1, sampler=sampler3, num_workers=self.num_worker)
        else:
            dataloader1 = data.DataLoader(self.train1_set, self.batch_size, sampler=sampler1, num_workers=self.num_worker)
            dataloader2 = data.DataLoader(self.train2_set, self.batch_size, sampler=sampler2, num_workers=self.num_worker)
            dataloader3 = data.DataLoader(self.train3_set, self.batch_size, sampler=sampler3, num_workers=self.num_worker)

        batch_per_epoch = batch_per_epoch or len(dataloader1)
        model = self.model
        model.split = "train"
        if self.world_size > 1:
            if self.device.type == "cuda":
        ...
        for epoch in self.meter(num_epoch):
            ...
            for batch_id, batch in enumerate(islice(dataloader, batch_per_epoch)):
                ...
                loss, metric = model(batch, eph=eph)
